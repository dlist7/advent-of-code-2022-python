...
def main():
    ...
    while not done and len(paths) > 0:
        ...
        for path in paths:
            ...
            # East
            np = (r,c+1)
            if c + 1 < cols - 1 and r < rows - 1 and not blizzard_map.get(np) and np not in pset:
                npath = path[:] + [np]
                npaths.append(npath)
                pset.add(np)
            ...
            # West
            np = (r,c-1)
            if c - 1 > 0 and r < rows - 1 and not blizzard_map.get(np) and np not in pset:
                npath = path[:] + [np]
                npaths.append(npath)
                pset.add(np)
