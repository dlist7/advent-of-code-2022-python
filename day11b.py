    ...
    for line in fileinput.input():
        ...
        if match:
            ...
            for i in range(0, len(current_monkey['items'])):
                current_monkey['items'][i] = int(current_monkey['items'][i])
    ...
    for i in range(0, 10000):
        for m in monkeys:
            for item in m['items']:
                ...
                new_item %= factor
                if m['test'](new_item):
                    monkeys[m['true']]['items'].append(new_item)
                else:
                    monkeys[m['false']]['items'].append(new_item)
            m['items'] = []
